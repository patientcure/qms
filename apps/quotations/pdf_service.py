...
class QuotationPDFGenerator:
    def __init__(self, quotation, items_data, user, company_profile=None, terms=None):
        ...
        self.doc = SimpleDocTemplate(
            ...
            leftMargin=20 * mm,
            topMargin=50 * mm,
            bottomMargin=40 * mm # Adjusted for footer content
        )
    ...
    def _draw_footer(self, canvas):
        ...
        try:
            ...
            if eureka_path:
                ...
                canvas.drawImage(eureka_logo, 60 * mm, 25 * mm, width=30 * mm, height=12 * mm, preserveAspectRatio=True, mask='auto')
        except Exception as e:
        ...
        try:
            ...
            if carysil_path:
                ...
                canvas.drawImage(carysil_logo, 120 * mm, 25 * mm, width=30 * mm, height=12 * mm, preserveAspectRatio=True, mask='auto')
        except Exception as e:
    ...
    def _define_styles(self):
        self.quotation_title_style = ParagraphStyle(
            'QuotationTitle', parent=self.styles['Heading2'], fontName='Helvetica-Bold',
            alignment=TA_CENTER, spaceAfter=6, textColor=self.dark_gray
        )
        self.section_heading_style = ParagraphStyle(
            'SectionHeading', parent=self.styles['Heading2'], fontSize=12,
            fontName='Helvetica-Bold', spaceAfter=8, spaceBefore=12, textColor=colors.black
        )
        self.normal_style = ParagraphStyle(
            'Normal', parent=self.styles['Normal'], fontSize=10, textColor=self.dark_gray, leading=14
        )
        self.right_style = ParagraphStyle(
            'Right', parent=self.normal_style, alignment=TA_RIGHT
        )
        self.terms_heading_style = ParagraphStyle(
            'TermsHeading', parent=self.styles['Heading3'], fontSize=11, fontName='Helvetica-Bold',
            spaceAfter=6, spaceBefore=6, textColor=colors.black
        )
        self.terms_content_style = ParagraphStyle(
            'TermsContent', parent=self.styles['Normal'], fontSize=9,
    ...
    def _build_header_info(self):
        """Builds centered "QUOTATION" title and right-aligned date."""
        return [
            Spacer(1, 4 * mm),
            Paragraph("QUOTATION", self.quotation_title_style),
            Paragraph(f"Date: {datetime.now().strftime('%d-%m-%Y')}", self.right_style),
            Spacer(1, 8 * mm)
        ]

    def _build_customer_info(self):
        """Builds customer info box with "To:" label."""
        ...
        if customer.email:
            info.append(f"<b>Email:</b> {customer.email}")
        if customer.phone:
            info.append(f"<b>Phone:</b> {customer.phone}")
        if customer.primary_address:
            info.append(f"<b>Address:</b> {customer.primary_address}")

        customer_details = "<br/>".join(info)
        p_customer_details = Paragraph(customer_details, self.normal_style)

        customer_table = Table([[p_customer_details]], colWidths=[170 * mm])
    ...
    def _build_items_table(self):
        """Build items table, hiding discount column if not used."""
        has_any_discount = any(self._to_decimal(item.get('discount', 0)) > 0 for item in self.items_data)

        if has_any_discount:
            headers = ['S.No.', 'Product/Service', 'Quantity', 'Rate', 'Discount', 'Amount']
            colWidths = [12*mm, 78*mm, 15*mm, 25*mm, 20*mm, 30*mm]
        else:
            headers = ['S.No.', 'Product/Service', 'Quantity', 'Rate', 'Amount']
            colWidths = [12*mm, 98*mm, 15*mm, 25*mm, 30*mm]
        ...
        for idx, item in enumerate(self.items_data, 1):
            ...
            row = [
                ...
                Paragraph(str(quantity), self.right_style),
                Paragraph(self._format_currency(unit_price), self.right_style),
            ]

            if has_any_discount:
                discount_display = f"{item_discount_percent:.0f}%" if item_discount_percent > 0 else "--"
                row.append(Paragraph(discount_display, self.right_style))
    ...
    def _build_totals(self, totals):
        ...
        tax_amount = subtotal_after_all_discounts * (tax_rate / 100) if tax_rate > 0 else Decimal('0.00')
        
        grand_total = subtotal_after_all_discounts + tax_amount

        totals_data = [['Subtotal:', self._format_currency(subtotal_after_item_disc)]]
        
        if overall_discount_value > 0:
            discount_label = f'Special Discount ({overall_discount_value:.0f}%):' if getattr(self.quotation, 'discount_type', 'percentage') != 'amount' else 'Special Discount:'
            totals_data.append([discount_label, f"(Rs. {overall_discount_amount:,.2f})"])

        if tax_rate > 0:
            totals_data.append([f'Tax ({tax_rate:.0f}%):', self._format_currency(tax_amount)])

        totals_data.append(['Total Amount:', self._format_currency(grand_total)])
    ...
    def _build_terms(self):
        """Build terms, conditions, and validity section."""
        elements = []
        terms_to_display = []
        ...
        if terms_to_display:
            ...
            for term in terms_to_display:
                ...
                if content:
                    bullet_points = [bp.strip() for bp in content.split('*') if bp.strip()]
                    list_items = [ListItem(Paragraph(bp, self.terms_content_style)) for bp in bullet_points]
                    elements.append(ListFlowable(list_items, bulletType='bullet', leftIndent=5 * mm))
        ...
        if self.quotation.follow_up_date:
            ...
            valid_until_table = Table([[valid_until_para]], colWidths=[170 * mm])
            valid_until_table.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, -1), colors.Color(254/255, 249/255, 195/255)),
                ('PADDING', (0, 0), (-1, -1), 8)
            ]))
            elements.append(Spacer(1, 5 * mm))
            elements.append(valid_until_table)
            
        return elements

    def _build_final_footer(self):
        """Builds the final centered thank you and created by notes."""
        creator_name = "Admin"
        ...
        return [
            Spacer(1, 15 * mm),
            Paragraph("Thank you for your business!", self.footer_style),
            Spacer(1, 1 * mm),
            Paragraph(f"Created By: {creator_name}", self.footer_style)
        ]
    ...
    def generate(self):
        ...
        elements.extend(self._build_header_info())
        ...
        elements.extend(self._build_final_footer())
